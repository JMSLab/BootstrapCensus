...
cd 'temp/K2021/Kaenzig_replication/codes'
...
bootIRFs = round(bootIRFs, n_digits);

OilPriceResponse = bootIRFs(:,1,:);
writematrix(OilPriceResponse, sprintf('%s/OilPriceResponse.csv', outdir))

OilProductionResponse = bootIRFs(:,2,:);
writematrix(OilProductionResponse, sprintf('%s/OilProductionResponse.csv', outdir))

OilInventoriesResponse = bootIRFs(:,3,:);
writematrix(OilInventoriesResponse, sprintf('%s/OilInventoriesResponse.csv', outdir))

WorldProductionResponse = bootIRFs(:,4,:);
writematrix(WorldProductionResponse, sprintf('%s/WorldProductionResponse.csv', outdir))

USProductionResponse = bootIRFs(:,5,:);
writematrix(USProductionResponse, sprintf('%s/USProductionResponse.csv', outdir))

CPIResponse = bootIRFs(:,6,:);
writematrix(CPIResponse, sprintf('%s/CPIResponse.csv', outdir))
...
% set(hh,'edgecolor','none'); 
% set(hh,'edgealpha',.4);
% f1=plot(sampleDasptesNum(smplStartProxyVARInd:smplEndProxyVARInd),varEst.Y(:,1)-mean(varEst.Y(:,1)),'Color',[0.8500, 0.3250, 0.0980],'LineWidth',2,'LineStyle',':');
